...
- [x] проверить обработку edge cases: пустые ответы API, невалидный JSON, отсутствие токена
- [x] запустить полный `go test ./...`
- [x] запустить `go vet ./...` — все замечания исправить
- [x] проверить что `go build` создаёт рабочий бинарник

### Task 8: Документация
- [x] написать README.md: описание, установка, использование, пример crontab
- [x] добавить пример `.env` файла
